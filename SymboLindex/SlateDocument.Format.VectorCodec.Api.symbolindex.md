...
%scope      folder
%path       Engine/SlateDocument/Format/VectorCodec/Api
%layer      SlateDocument
%sources    1
%symbols    5
%annotated  3/5
...
S VectorCodec.h | 73 lines | 91bb3380 | 5 sym | `10` §1 — vector streams translated into `52`'s accepted subset, with every refusal named and positioned.

//------------------------------------------------------------------------------------------------------------------------
//                                                  WHAT A DECODE YIELDS
//------------------------------------------------------------------------------------------------------------------------

T DecodedOutline           | VectorCodec.h | 28-32 | owning          | -  | One decoded vector source — the outline, and every construct the accepted subset would not take. unsupported construct still decodes; refusing the whole of it would lose the artist the ninety-nine paths that were fine. The whole point of positioning a refusal is that the artist can go and look.
    has   Declared  OutlineSpecification           [-]  ?
    has   Refused   std::vector<RefusedConstruct>  [-]  ?
...
F SLATE_DECLARES_PRECISION | VectorCodec.h | 56    | -               | -  | ?
...
F SLATE_DECLARES_PRECISION | VectorCodec.h | 69    | -               | -  | ?
    in    Bounded  PrecisionGuarantee::  [-]  ?
    in    Bounded  PrecisionGuarantee::  [-]  ?
    in    Exact    PrecisionGuarantee::  [-]  ?
    by    Api/AnalyticProjection.h, Api/AssetInterchange.h, Api/AtmosphereIntegrator.h, Api/BrushSpecification.h, Api/CameraProjection.h, Api/ChannelPanel.h, (+50 more)
