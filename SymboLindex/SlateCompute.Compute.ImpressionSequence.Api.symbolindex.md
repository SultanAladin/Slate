...
%scope      folder
%path       Engine/SlateCompute/Compute/ImpressionSequence/Api
%layer      SlateCompute
%sources    1
%symbols    26
%annotated  14/26
...
T StrokeDeclaration                       | ImpressionSequence.h | 95-104  | owning                        | -  | Everything a stroke needs before its first impression.
    has   Placements      std::vector<ChannelPlacement>  [-]  ?
    has   Subject         LayerIdentity                  [-]  ?
    has   SurfaceOrdinal  std::uint32_t                  [-]  ?
    has   WorkingExtent   std::uint32_t                  [-]  ?
...
T ResolvedRun                             | ImpressionSequence.h | 115-120 | nonallocating,nonthrowing     | -  | How one rotation's resolution ended. reasoning is identical one layer up: an impression waiting for a tile is ordinary operation, and a register that appended one per impression per rotation is a register nobody reads.
...
T ImpressionSequence                      | ImpressionSequence.h | 170-307 | owning                        | -  | One stroke — its resampled path, its impressions, its accumulation, and the one transaction it seals. Abandon ends with no effect, Seal enters **one** transaction. A stroke recording a transaction per pointer sample fills the revision sequence with positions the artist never intended to stop at, and undo then steps back one pixel at a time. resampled against consumption has the display rate baked into it, so the same physical gesture produces a different stroke on a machine that presents at a different rate.
    ...
    has   Recorded           StrokeBrushRecord               [-]  ?
    has   Sequenced          std::vector<ImpressionSample>   [-]  ?
    has   Accumulated        StrokeSpace                     [-]  ?
    has   Combination        CombineSpecification            [-]  ?
    has   LastAlong          double                          [-]  ?
    has   LastAcross         double                          [-]  ?
    has   LastArrival        TickPoint                       [-]  ?
    has   TravelledDistance  double                          [-]  ?
    has   PendingDistance    double                          [-]  ?
    has   NextSpacing        double                          [-]  ?
    has   Level              std::uint32_t                   [-]  ?
    has   ResolvedTotal      std::uint32_t                   [-]  ?
