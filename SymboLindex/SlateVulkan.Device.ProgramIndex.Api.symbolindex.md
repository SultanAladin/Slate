...
%scope      folder
%path       Engine/SlateVulkan/Device/ProgramIndex/Api
%layer      SlateVulkan
%sources    1
%symbols    15
%annotated  11/15
...
S ProgramIndex.h | 191 lines | a7ee2b54 | 15 sym | Graphics and compute programs constructed once at bring-up, against the layouts and modules already declared.

//------------------------------------------------------------------------------------------------------------------------
//                                                    WHAT IS DECLARED
//------------------------------------------------------------------------------------------------------------------------

V AbsentProgram                 | ProgramIndex.h | 27      | -                             | -  | ?
    by    Api/OcclusionScheduler.h, Api/VisibilityRaster.h, Source/OcclusionScheduler.cpp, Source/VisibilityRaster.cpp

T DepthDeclaration              | ProgramIndex.h | 36-41   | nonallocating,nonthrowing     | -  | How one graphics program resolves depth — tested, written, and against which comparison. and `FarPlaneDepth` is nought. A program declaring the ordinary less-than comparison against a reversed target resolves the furthest surface at every pixel, and the image is the inside of the object rather than an image that sorts wrongly — which is the failure `02` §6 declares the two constants to prevent.
    has   DepthTested      bool         [-]  ?
    has   DepthWritten     bool         [-]  ?
    has   DepthComparison  VkCompareOp  [-]  ?
    note  🔴 The comparison defaults to `VK_COMPARE_OP_GREATER` because `Contract/`'s `NearPlaneDepth` is unity

T GraphicsDeclaration           | ProgramIndex.h | 54-67   | owning                        | -  | One graphics program, as the recording that constructs it declares it. declared spans by the vertex ordinal, which is what lets one program serve every partitioning without a per-topology input declaration — and what keeps the attribute set out of a document that writes no attribute. and a program that combined its output with what stood there would be reading a target it produces. here. A program naming a second would be constructed against a division `AttachmentIndex` does not declare, and the vendor reports that at the draw rather than at the construction.
    has   VertexModule           std::uint32_t                     [-]  ?
    has   FragmentModule         std::uint32_t                     [-]  ?
    has   LayoutOrdinals         std::vector<std::uint32_t>        [-]  ?
    has   VertexFixed            std::vector<SpecialisedConstant>  [-]  ?
    has   FragmentFixed          std::vector<SpecialisedConstant>  [-]  ?
    has   RenderConstruct        VkRenderPass                      [-]  ?
    has   ColourAttachmentCount  std::uint32_t                     [-]  ?
    has   ConstantBytes          std::uint32_t                     [-]  ?
    has   Assembled              VkPrimitiveTopology               [-]  ?
    has   FacingCulled           VkCullModeFlags                   [-]  ?
    has   Depth                  DepthDeclaration                  [-]  ?
...
T ComputeDeclaration            | ProgramIndex.h | 74-80   | owning                        | -  | One compute program, as the dispatching document declares it. specialised constant supplied through `Fixed` — `06` §2.1's reason for admitting specialisation at all, and either way it is the shader's declaration rather than a second one held beside it.
    has   ModuleOrdinal   std::uint32_t                     [-]  ?
    has   LayoutOrdinals  std::vector<std::uint32_t>        [-]  ?
    has   Fixed           std::vector<SpecialisedConstant>  [-]  ?
    has   ConstantBytes   std::uint32_t                     [-]  ?
...
T ConstructedProgram            | ProgramIndex.h | 91-96   | nonallocating,nonthrowing     | -  | What a recording is handed — the program, the layout its descriptors reach through, and how it is recorded. recording writes is written **through** it. A recording resolving the program alone would then reach for a layout it derived separately, and two derivations of one layout is `00` §2's case exactly.
    has   Constructed    VkPipeline           [-]  ?
    has   ReachedLayout  VkPipelineLayout     [-]  ?
    has   RecordedAs     VkPipelineBindPoint  [-]  ?
    by    Api/VisibilityRaster.h, Source/OcclusionScheduler.cpp, Source/ProgramIndex.cpp, Source/VisibilityRaster.cpp
...
F ProgramIndex::~ProgramIndex   | ProgramIndex.h | 121     | destructor                    | -  | ?

F ProgramIndex::Construct       | ProgramIndex.h | 131     | api,nonthrowing               | ✔️ | Takes the device, the modules every program is constructed from, and the layouts it reaches through.
...
F ProgramIndex::DeclareGraphics | ProgramIndex.h | 142     | api,nonthrowing               | 🔴 | Constructs one graphics program, returning the ordinal every later resolution names it by. an absent render construct, and with HostDenied when the device declines it specialisation the vendor reads at construction is held where its addresses stay put.
...
F ProgramIndex::DeclareCompute  | ProgramIndex.h | 150     | api,nonthrowing               | 🔴 | Constructs one compute program, returning the ordinal every later resolution names it by.
...
F ProgramIndex::Resolve         | ProgramIndex.h | 157     | api,nonallocating,nonthrowing | ✔️ | The program one ordinal names, for the recording that records against it.
...
F ProgramIndex::Reclaim         | ProgramIndex.h | 163     | api,nonthrowing               | 🚩 | Destroys every program and every layout constructed for one.
    out   -  void  [-]  ?
...
F ProgramIndex::DeclaredCount   | ProgramIndex.h | 165     | -                             | -  | ?
    out   -  std::uint32_t  [-]  ?
    by    Api/AttachmentIndex.h, Api/BrushSpecification.h, Api/DecalProjection.h, Api/DescriptorIndex.h, Api/MaterialSpecification.h, Api/QuadratureIntegrator.h, (+19 more)

T ProgramIndex::HeldProgram     | ProgramIndex.h | 169-174 | -                             | -  | ?
    has   Constructed    VkPipeline           [-]  ?
    has   ReachedLayout  VkPipelineLayout     [-]  ?
    has   RecordedAs     VkPipelineBindPoint  [-]  ?
    by    Source/ProgramIndex.cpp

F ProgramIndex::ReachLayout     | ProgramIndex.h | 181     | -                             | -  | Constructs the layout one program reaches its declared sets and its constant run through.
