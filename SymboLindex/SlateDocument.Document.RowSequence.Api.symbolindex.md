...
S RowSequence.h | 182 lines | 0b1eb09f | 18 sym | Depth-first linearisation of the enclosure relation, and the counted ordering that scrolls it.
...
T RowSequence                    | RowSequence.h | 107-180 | owning                        | -  | The rows the enclosure relation linearises to, paired with the ordering that scrolls them. because rows rebuilt against stale labels are briefly wrong and are displayed while they are.
    has   SequencedRows    std::vector<SequencedRow>   [-]  ?
    has   RowOfSlot        std::vector<std::uint32_t>  [-]  ?
    has   SlotCollapsed    std::vector<bool>           [-]  ?
    has   SlotRetained     std::vector<bool>           [-]  ?
    has   VisibleOrdering  RankIndex                   [-]  ?
    has   NarrowingHeld    bool                        [-]  ?
...
F RowSequence::Rows              | RowSequence.h | 146     | api,nonallocating,nonthrowing | ✔️ | The rows, in linearised order, including the uncounted ones.
    out   -  const std::vector<SequencedRow>&  [-]  ?
    by    Source/CameraProjection.cpp, Source/OutlinerPanel.cpp, Source/RowSequence.cpp
...
F RowSequence::CountsAgree       | RowSequence.h | 168     | api,nonallocating,nonthrowing | 🚩 | 🔍 Whether the counts agree with the visible rows of the sequence — invariant 5.
    out   -  bool  [-]  ?
    by    Source/OutlinerSequence.cpp, Source/RowSequence.cpp

F RowSequence::Recount           | RowSequence.h | 172     | -                             | -  | ?
    out   -  void  [-]  ?
    by    Source/RowSequence.cpp
