...
E SharedTarget               | RenderSchedule.h | 26-44   | contract                      | -  | Every shared target the schedule declares. Nothing invents a target another already produces.
...
F RelationOfTarget           | RenderSchedule.h | 62      | api,nonallocating,nonthrowing | ✔️ | The relation one target's extent bears to the display extent. a relation from a format or an extent has derived it from the wrong operand.
...
T TargetSpace                | RenderSchedule.h | 75-142  | owning                        | -  | What the fifteen declared targets are claimed as — one image ordinal each, against one display extent. fifteen formats and their extent relations and nothing walked it; `TargetSpace` is what walks it. and fraction-of-display target and touches no absolute one. `Reclaim` refuses to hold a persistent extent across the change, and an intermediate drag extent is discarded by the caller not queued here.
    has   ImageEdge       ImageSpace*    [-]  ?
    has   StandingWidth   std::uint32_t  [-]  ?
    has   StandingHeight  std::uint32_t  [-]  ?
    has   DisplayCarries  VkFormat       [-]  ?
    by    Api/AttachmentIndex.h, Api/OcclusionScheduler.h, Source/AttachmentIndex.cpp, Source/OcclusionScheduler.cpp, Source/RenderSchedule.cpp
...
F TargetSpace::Claim         | RenderSchedule.h | 95      | api,nonthrowing               | 🔴 | Claims every declared target against one display extent and one display format. whatever `ImageSpace` refused when a target could not be claimed was never claimed, and the recording site meets it as a null view rather than as this refusal.
    in    Images         ImageSpace&    [-]   where the images are claimed; borrowed and outlives this component
    in    DisplayWidth   std::uint32_t  [px]  the display extent this claim is relative to
    in    DisplayHeight  std::uint32_t  [px]  the display extent this claim is relative to
...
F TargetSpace::OrdinalOf     | RenderSchedule.h | 122     | api,nonallocating,nonthrowing | ✔️ | The image ordinal one target was claimed as, for the transition `ImageSpace` records.
    in    Target  SharedTarget  [-]  ?
    out   -       Deliver       [-]  refuses with ContentUnsupported when the target is unclaimed
    by    Api/SurfaceTileSpace.h, Source/ImpressionSequence.cpp, Source/OcclusionScheduler.cpp, Source/RenderSchedule.cpp, Source/SurfaceTileSpace.cpp
