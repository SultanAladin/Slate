...
3. **Shared authoring dispatch**
   ...
   - Status: in progress; the shared entry point now handles line, polyline, rectangle, circle, three-point arc, center/start/end arc, tangent arc, elliptical arc, Bézier, basis spline, rational spline, Hermite, polygon, and slot records. The shared request now carries snap placements; dimension/constraint resolution, complete snap solving, and preview rendering still need extraction from the Parametric host.
