...
- Hardening pass added dedicated constraint toolbar entries for horizontal, vertical, coincident, parallel, perpendicular, and tangent constraints; applying them creates constraint rows and invokes the constraint solver immediately.
- Viewport validation now reports valid profiles and constraint health in the sketch viewport.
- Visual/code validation proof lives at `References/Cad/SketchHardeningValidation.svg` and is generated by `Tools/ValidateSketchHardening.py`.
- Next: strengthen exact trim/extend beyond single-curve cut preparation, add full tangent/perpendicular snap projection in the C++ snap resolver, and add richer profile-loop diagnostics.
