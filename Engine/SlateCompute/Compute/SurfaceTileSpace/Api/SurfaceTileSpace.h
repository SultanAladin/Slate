...
public:
    ...
    /// cost  ✔️
    /// tag   api, nonthrowing
    Outcome<bool> Evict(std::uint32_t CellOrdinal, std::uint64_t RotationOrdinal);
...
private:
    ...
    Outcome<std::uint32_t> ClaimOrEvict(PromotionScheduler& Scheduling, std::uint64_t RotationOrdinal);
