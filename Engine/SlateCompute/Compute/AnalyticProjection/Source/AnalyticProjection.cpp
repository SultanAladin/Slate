...
Outcome<bool> AnalyticProjection::Construct(const AnalyticSources& Supplied_)
{
    Supplied = Supplied_;
                                                             ...
                                                             double         Tolerance) const
{
    ...
    if (Declared.Classify(*Flattened, SourceAlong, SourceAcross) < 0)
        return Outcome<ResolvedSample>::Deliver(Resolved);
                                                         ...
                                                         double         Tolerance) const
{
    ...
    for (std::size_t Ordinal = 0u; Ordinal < Declared.GlyphSequence.size(); ++Ordinal)
    {
        ...
        if (!Held.ContentPresent)
            return Outcome<ResolvedSample>::Refuse(Held.Declined);
                                                            ...
                                                            std::uint32_t  NestingDepth) const
{
    ...
    const Outcome<const TilingSpecification*> Declared = Supplied.Tilings->Resolve(TilingOrdinal);

    if (!Declared.ContentPresent)
        return Outcome<ResolvedSample>::Refuse(Declared.Declined);
    ...
    if (!Classified.ContentPresent)
        return Outcome<ResolvedSample>::Refuse(Classified.Declined);
                                                            ...
                                                            double         Tolerance) const
{
    ...
    const Outcome<const PlacementSpecification*> Declared = Supplied.Placements->Resolve(PlacementOrdinal);
    ...
    if (!ProjectIntoSource(Placed, PositionAlong, PositionAcross, SourceAlong, SourceAcross))
        return Outcome<ResolvedSample>::Deliver(ResolvedSample{});
...
std::uint64_t AnalyticProjection::ContentRevision(const SurfaceLayerSequence& Content) const
{
    ...
    {
        const Outcome<const SurfaceLayerSequence*> Nesting = Content.Nested(NestedOrdinal);
                                                      ...
                                                      std::uint32_t                         ComponentCount) const
{
    ...
    for (const LayerSpecification& Held : Content.Entries())
    {
        ...
        {
            const Outcome<const SurfaceLayerSequence*> Nesting = Content.Nested(Held.NestedOrdinal);

            if (!Nesting.ContentPresent)
                return Outcome<ResolvedSample>::Refuse(Nesting.Declined);
                                                      ...
                                                      std::uint32_t                         ComponentCount) const
{
    ...
    for (std::uint32_t Across = 0u; Across < StoredTexelsPerEdge; ++Across)
    {
        for (std::uint32_t Along = 0u; Along < StoredTexelsPerEdge; ++Along)
        {
            ...
            if (!Sampled.ContentPresent)
            {
                ...
                return Outcome<ResolvedTile>::Refuse(Sampled.Declined);
            }
    ...
    return Outcome<ResolvedTile>::Deliver(Produced);
}
