...
Outcome<GeometryAssetView> GeometryFileInterchange::Import(const std::string& OriginPath,
                                                            ...
                                                            IntakeIndex& Intake) const
{
    ...
    {
        return Outcome<GeometryAssetView>::Refuse(
    ...
    const Outcome<DecodedTopology> Decoded = Formats.Decode(Stream, OriginPath);
    if (!Decoded.Resolved)
        return Outcome<GeometryAssetView>::Refuse(Decoded.Error);

    const Outcome<GeometryIdentity> Accepted = Geometry.AcceptDecoded(Decoded.Resolve(), Naming, Intake);
    if (!Accepted.Resolved)
        return Outcome<GeometryAssetView>::Refuse(Accepted.Error);
