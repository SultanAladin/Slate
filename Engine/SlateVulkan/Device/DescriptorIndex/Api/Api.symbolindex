...
S DescriptorIndex.h | 174 lines | f2cd451a | 18 sym | Descriptor set layouts constructed once at bring-up, and explicit sets claimed one per rotation slot.

//------------------------------------------------------------------------------------------------------------------------
//                                                   THE DECLARED SHAPE
//------------------------------------------------------------------------------------------------------------------------

V AbsentDescriptor                  | DescriptorIndex.h | 26      | -                             | -  | ?
    by    Api/OcclusionScheduler.h, Api/VisibilityRaster.h

T DescriptorSlot                    | DescriptorIndex.h | 32-38   | nonallocating,nonthrowing     | -  | What one descriptor slot in a layout carries, as the shader declares it. layout must state, and a layout stating a larger one leaves the shader indexing beyond what is written.
    ...
    by    Api/GlyphDepot.h, Api/InterfaceExchange.h, Source/DescriptorIndex.cpp, Source/GlyphDepot.cpp, Source/InterfaceExchange.cpp, Source/OcclusionScheduler.cpp, (+1 more)
    note  🔴 The count is a declaration and not a ceiling. A shader declaring an unsized run reads a count the

T DescriptorContent                 | DescriptorIndex.h | 45-54   | nonallocating,nonthrowing     | -  | What one descriptor set is written with — one entry per slot the recording amends. sampled image into a slot the layout declares as a span is a validation error at the write rather than at the declaration that disagreed.
    has   SlotOrdinal    std::uint32_t  [-]  ?
...
F DescriptorIndex::~DescriptorIndex | DescriptorIndex.h | 75      | destructor                    | -  | ?
...
F DescriptorIndex::Amend            | DescriptorIndex.h | 124     | api,nonthrowing               | 🚩 | Writes the content of one claimed set for one rotation slot. or above the depth, or a slot the layout does not declare
...
F DescriptorIndex::DeclaredCount    | DescriptorIndex.h | 146     | -                             | -  | ?
    out   -  std::uint32_t  [-]  ?
    by    Api/AttachmentIndex.h, Api/BrushSpecification.h, Api/DecalProjection.h, Api/GlyphDepot.h, Api/HardwareMetrics.h, Api/MaterialSpecification.h, (+36 more)

F DescriptorIndex::ClaimedCount     | DescriptorIndex.h | 147     | -                             | -  | ?
    out   -  std::uint32_t  [-]  ?
...
T DescriptorIndex::DeclaredLayout   | DescriptorIndex.h | 151-155 | -                             | -  | ?
    has   Constructed  VkDescriptorSetLayout        [-]  ?
    has   Slots        std::vector<DescriptorSlot>  [-]  ?
    by    Source/DescriptorIndex.cpp

T DescriptorIndex::ClaimedSet       | DescriptorIndex.h | 157-161 | -                             | -  | ?
    has   LayoutOrdinal  std::uint32_t                 [-]  ?
    has   PerRotation    std::vector<VkDescriptorSet>  [-]  ?
    by    Source/DescriptorIndex.cpp

F DescriptorIndex::SlotOf           | DescriptorIndex.h | 164     | -                             | -  | Which declared slot carries an ordinal, or nothing when the layout does not declare it.
    in    Holding      const DeclaredLayout&  [-]  ?
    in    SlotOrdinal  std::uint32_t          [-]  ?
    out   -            const DescriptorSlot*  [-]  ?
    by    Api/OcclusionProjection.h, Source/DescriptorIndex.cpp, Source/OcclusionProjection.cpp
