...
Outcome<VkDescriptorSet> DescriptorIndex::Resolve(std::uint32_t ClaimOrdinal, std::uint32_t RotationSlot) const
{
    if (static_cast<std::size_t>(ClaimOrdinal) >= Claimed.size())
    ...
    return Outcome<VkDescriptorSet>::Deliver(Claimed[ClaimOrdinal].PerRotation[RotationSlot]);
}
...
Outcome<VkDescriptorSetLayout> DescriptorIndex::Layout(std::uint32_t LayoutOrdinal) const
{
    ...
    return Outcome<VkDescriptorSetLayout>::Deliver(Layouts[LayoutOrdinal].Constructed);
}
