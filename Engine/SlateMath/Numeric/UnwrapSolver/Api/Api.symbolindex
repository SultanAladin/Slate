...
%scope      folder
%path       Engine/SlateMath/Numeric/UnwrapSolver/Api
%layer      SlateMath
%sources    1
%symbols    6
%annotated  4/6
...
S UnwrapSolver.h | 97 lines | 9e6a4724 | 6 sym | Boundary-first parameterisation — Convergent, and held to reporting which criterion terminated it.

//------------------------------------------------------------------------------------------------------------------------
//                                                     WHAT IS SOLVED
//------------------------------------------------------------------------------------------------------------------------

T UnwrapSpecification      | UnwrapSolver.h | 31-38 | owning                    | -  | One chart handed to the solver — its positions, its triangulation, and its boundary loop. property in object space, so a chart flattens identically wherever the occupant sits and an artist moving a scene re-derives nothing here. and cannot be flattened at all; the caller resolves that by subdividing, which is `68` §4.1's response to a fold and is deliberately the same mechanism.
