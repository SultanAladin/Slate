...
Result<bool> PlatformInterchange::Resolve()
{
    if (ReportDelivered)
    ...
    if (GlobalMemoryStatusEx(&InstalledExtent) == FALSE)
        return Result<bool>::Refuse({ RefusalReason::HostDenied, "the host declined to report its extent" });
...
Result<std::string> PlatformInterchange::ExecutableDirectory()
{
#if defined(_WIN32)
    ...
    if (Spanned == 0u || Spanned >= PathCapacity)
        return Result<std::string>::Refuse({ RefusalReason::HostDenied, "the host declined the executable path" });
    ...
    if (Separated == std::wstring::npos)
        return Result<std::string>::Refuse({ RefusalReason::HostDenied, "the executable path names no directory" });
    ...
    if (Narrowed <= 0)
        return Result<std::string>::Refuse({ RefusalReason::HostDenied, "the executable path is not representable" });
    ...
    return Result<std::string>::Result(Narrow);
    ...
    return Result<std::string>::Result(Located);

#endif
}

Result<std::string> PlatformInterchange::RetainedDirectory(const char* ApplicationName)
{
    if (ApplicationName == nullptr)
        return Result<std::string>::Refuse({ RefusalReason::HostDenied, "no application name was declared" });
    ...
    if (SHGetKnownFolderPath(FOLDERID_RoamingAppData, 0, nullptr, &Located) != S_OK)
    {
        ...
        return Result<std::string>::Refuse({ RefusalReason::HostDenied, "the host declined its retained directory" });
    }
    ...
    {
        CoTaskMemFree(Located);
        return Result<std::string>::Refuse({ RefusalReason::HostDenied, "the retained path is not representable" });
    }
    ...
    if (Widths > 0)
    {
        ...
        if (CreateDirectoryW(Widened.c_str(), nullptr) == FALSE && GetLastError() != ERROR_ALREADY_EXISTS)
            return Result<std::string>::Refuse({ RefusalReason::HostDenied, "the retained directory was declined" });
    }
    ...
    else
    {
        ...
        if (HomeDirectory == nullptr || HomeDirectory[0] == '\0')
            return Result<std::string>::Refuse({ RefusalReason::HostDenied, "the host declared no home directory" });
